...
# parts 1,2,3,4,5,6,9,10,20 to keep original mouth
...
def video_swap(source_image, target_video, result_video, enhance_mode):
    ...
    swapper = insightface.model_zoo.get_model('inswapper_128/inswapper_128.onnx', download=True, download_zip=True)
    ...
    if w_out %2 !=0 : w_out = w_out - 1
    if h_out %2 !=0 : h_out = h_out - 1
    ...
    for frame_idx in tqdm(range(cutout)):
        ...
        #cv2.imshow("Rot1",face_region)
                    
        detect_results = app.get(face_region)
        ...
        if detect_results is not None:
            ...
            try:
                swapped_face, matrix = swapper.get(res, detect_results[0], source_face, paste_back=False)
                #cv2.imshow("S",swapped_face)
                #for enhancment blending
                swapped_orig = swapped_face.copy()
                swapped_orig = cv2.resize(swapped_orig,(256,256))
                #cv2.imshow("S",swapped_orig)
            
                mat_rev = cv2.invertAffineTransform(matrix)
            
                #for pasting back enhanced swapped_face 256x256
                facematrix = np.multiply(matrix,2)
                facematrix_rev = cv2.invertAffineTransform(facematrix)

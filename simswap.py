...
simswap_path = "face_swapper/simswap.onnx"
swapper = SimSwap(simswap_path, **onnx_params)
simswap_backbone_path = "face_swapper/simswap_arcface_backbone.onnx"
    ...
    for frame_idx in tqdm(range(cutout)):
        ...
        try:
            ...
            if opt.segmentation:
                ...
                r_mask = cv2.cvtColor(r_mask, cv2.COLOR_GRAY2RGB)
                r_mask = cv2.rectangle(r_mask,(0,0),(256,10),(0,0,0), -1)
                r_mask = cv2.rectangle(r_mask,(10,246),(246,256),(0,0,0), -1)
                if opt.no_chin:
                    r_mask = cv2.rectangle(r_mask,(10,165),(246,256),(0,0,0), -1)
                r_mask = cv2.resize(r_mask,(224,224))
                r_mask = cv2.GaussianBlur(r_mask,(19,19),cv2.BORDER_DEFAULT)
